...
declare -A KEYS=(
    ["0xf702-0x280000"]="10:b"          # Option+Left → Esc+b (word back)
    ["0xf703-0x280000"]="10:f"          # Option+Right → Esc+f (word forward)
    ["0xf728-0x80000"]="10:d"           # Option+Delete → Esc+d (del word fwd)
    ["0xf728-0x0"]="11:0x4"             # Delete fwd → Ctrl+D
    ["0xf702-0x300000"]="11:0x1"        # Cmd+Left → Ctrl+A (line start)
    ["0xf703-0x300000"]="11:0x5"        # Cmd+Right → Ctrl+E (line end)
    ["0x7f-0x80000"]="11:0x1b 0x7f"     # Option+Backspace → Esc+Del (del word back)
    ["0x7f-0x100000"]="11:0x15"         # Cmd+Backspace → Ctrl+U (kill line)
)

for key in "${!KEYS[@]}"; do
    IFS=: read -r action text <<< "${KEYS[$key]}"
    path="${PROFILE}:Keyboard Map:${key}"
    # Delete and recreate to ensure correct type
